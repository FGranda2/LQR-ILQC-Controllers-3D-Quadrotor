...
%while ( i <= Task.max_iteration)    
    ...
    % =====================================================================
    % Forward Pass
    for j = 1:size(Controller.theta,3)-1
        if j == 1
            sim_out.x(:,j) = Task.start_x;
            x_k = sim_out.x(:,j);
            dx_k = x_k - x_lin1;
            u_k = Controller.theta(:,:,j).'*[1;x_k];
            sim_out.u(:,j) = u_k;
            du_k = u_k - u_lin1;
            dx_knext = A_k1*dx_k + B_k1*du_k;
            sim_out.x(:,j+1) = dx_knext + x_lin1;
        else
            x_k = sim_out.x(:,j);
            dx_k = x_k - x_lin1;
            u_k = Controller.theta(:,:,j).'*[1;x_k];
    ...
    for k = N-1:-1:1
        ...
        s_NN(:,k) = q_kk + A_k.'*s_NN_in+K_k.'*H_k*du_ff+K_k.'*g_k+G_k.'*du_ff;
        S_N(:,:,k) = Q_k + A_k.'*S_N_in*A_k + K_k.'*H_k*K_k + K_k.'*G_k + G_k.'*K_k;
    ...
    Controller.theta = Controller.theta;
    %}
    i = i+1;
end
